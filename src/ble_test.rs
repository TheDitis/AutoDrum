...
use bluer::{adv::Advertisement, Device, gatt::local::{
...
const SERVICE_UUID: uuid::Uuid = uuid::uuid!("03B80E5A-EDE8-4B33-A751-6CE34EC4C700");

/// Characteristic UUID for GATT example.
const CHARACTERISTIC_UUID: uuid::Uuid = uuid::uuid!("7772E5DB-3868-4112-A1A9-F2669D106BF3");
                    ...
                    fun: {
                        ...
                        Box::new(move |req| {
                            ...
                            async move {
                                ...
                                if matches!(vec![10, 1, 1, 10], value) {
                                    println!("This is a Pairing request!");
                                    let device: Device = adapter.device(req.device_address).unwrap();

                                    match device.connect().await {
                                        Ok(_) => println!("Connected to device {:?}", device),
                                        Err(e) => println!("Error connecting to device {:?}", e),
                                    }
                                }
                                // If the value matches the pairing request, return an empty vector
                                if value == vec![10, 1, 1, 10] {
                                    Ok(vec![])
                                } else {
                                    Ok(vec![])
                                }
...
// Value received on attempted pair: [10, 1, 1, 10]
